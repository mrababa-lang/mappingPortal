...
import { X, Loader2, Sparkles, ChevronLeft, ChevronRight, ChevronDown, Search, Check, Info, FileX, Clock, Download } from 'lucide-react';
...
export const Select = React.forwardRef<HTMLSelectElement, SelectProps>(({ label, options, error, className = '', ...props }, ref) => (
  <div className="space-y-1.5">
    ...
    <div className="relative">
      <select 
        ref={ref}
        className={`w-full px-4 py-2.5 bg-slate-50 border rounded-lg text-slate-900 text-sm appearance-none focus:outline-none focus:ring-2 transition-all ${
          error ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-slash-red/20 focus:border-slash-red/50'
        } ${className}`}
        {...props}
      >
...
// --- SearchableSelect (Functional Version) ---
...
export const SearchableSelect: React.FC<SearchableSelectProps> = ({ 
  label, options, value, onChange, placeholder = "Search...", disabled, error, className = '' 
}) => {
  ...
  return (
    <div className={`space-y-1.5 relative ${className}`} ref={wrapperRef}>
      ...
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-2.5 bg-slate-50 border rounded-lg text-sm text-left ${
          error ? 'border-red-300 focus:ring-red-200' : 'border-slate-200 focus:ring-slash-red/20 focus:border-slash-red/50'
              ...
              filtered.map(opt => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors flex items-center justify-between ${
                    opt.value == value ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'hover:bg-slate-50 text-slate-700'
                  }`}
                  onClick={() => {
